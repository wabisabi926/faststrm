...
func TestPhase3_RepoIntegration(t *testing.T) {
	...
	db, err := OpenNew(root)
	if err != nil { t.Fatalf("OpenNew err: %v", err) }
	defer db.Close()
	...
	t.Run("TaskHistoryRepo CRUD + pagination", func(t *testing.T) {
		...
		if err != nil { t.Fatalf("NewTaskHistoryRepo err: %v", err) }
		ctx := context.Background()

		// —— 初始为空
		qEmpty, err := repo.Query(ctx, TaskHistoryQuery{Limit: 10})
		if err != nil { t.Fatalf("Query empty err: %v", err) }
		if len(qEmpty) != 0 { t.Errorf("empty want 0 rows, got %d", len(qEmpty)) }
		...
		if err != nil { t.Fatalf("Create exec1 err: %v", err) }
		...
		if err != nil { t.Fatalf("Query by task err: %v", err) }
		if len(byTask) != 2 { t.Fatalf("Query task_A want 2, got %d: %+v", len(byTask), byTask) }
		...
		if byTask[0].DurationMs != 1234 { t.Errorf("duration want 1234, got %d", byTask[0].DurationMs) }
		...
		if err != nil { t.Fatalf("Query status failed err: %v", err) }
		...
		if err != nil { t.Fatalf("Query bob err: %v", err) }
		...
		if err != nil { t.Fatalf("Query limit=1 err: %v", err) }
		...
		if err != nil { t.Fatalf("GetLogs err: %v", err) }
		if len(logs) != 3 { t.Errorf("GetLogs limit=3 want 3, got %d", len(logs)) }
		for i, l := range logs {
			want := fmt.Sprintf("%04d-startup line %d", i, i*100)
			if l != want { t.Errorf("log[%d] mismatch: want %q, got %q", i, want, l) }
		}

		// —— GetLogs no limit：默认 20000，取 5
		logsAll, err := repo.GetLogs(ctx, id1, 0)
		if err != nil { t.Fatalf("GetLogs(0) err: %v", err) }
		if len(logsAll) != 5 { t.Errorf("GetLogs(0) want 5, got %d", len(logsAll)) }
	})

	t.Run("LifeEventRepo insert + ListPending + MarkProcessed", func(t *testing.T) {
		repo, err := NewLifeEventRepo(db)
		if err != nil { t.Fatalf("NewLifeEventRepo err: %v", err) }
		ctx := context.Background()
		...
		for _, e := range events {
			...
			if err != nil { t.Fatalf("Insert err: %v", err) }
			ids = append(ids, id)
		}
		if len(ids) != 5 { t.Fatalf("ids len: %d", len(ids)) }

		// ListPending(alice) → 3 条，按 createdAt ASC → 10/30/50
		alice, err := repo.ListPending(ctx, "alice@115.com", 20)
		if err != nil { t.Fatalf("ListPending alice err: %v", err) }
		if len(alice) != 3 { t.Fatalf("alice pending want 3, got %d", len(alice)) }
		...
		if err != nil { t.Fatalf("ListPending alice after mark err: %v", err) }
		...
		if err != nil { t.Fatalf("ListPending all err: %v", err) }
		if len(all) != 3 { t.Errorf("all pending want 3, got %d", len(all)) }
		// ASC：20(bob), 40(bob), 50(alice)
		for i, want := range []int64{20, 40, 50} {
			if all[i].CreatedAt != want { t.Errorf("all[%d].CreatedAt want %d, got %d", i, want, all[i].CreatedAt) }
		}

		// ListPending limit=1 → 仅最早 (bob createdAt=20)
		small, err := repo.ListPending(ctx, "", 1)
		if err != nil { t.Fatalf("ListPending limit 1 err: %v", err) }
