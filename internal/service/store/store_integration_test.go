...
func TestPhase3_StoreIntegration(t *testing.T) {
	...
	t.Run("AccountStore round-trip with encryption", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("ReadAccounts err: %v", err)
		}
		if len(back) != 2 {
		...
		if alice, ok := byAcct["alice@115.com"]; !ok {
			...
			if alice.Password != "p@ssw0rd" {
				t.Errorf("alice password mismatch: %q", alice.Password)
			}
			if alice.Cookie != "cookie_a=xxx; UID=u1" {
				t.Errorf("alice cookie mismatch: %q", alice.Cookie)
			}
			if alice.Name != "Alice" {
				t.Errorf("alice name mismatch")
			}
		}
		if bob, ok := byAcct["bob@115.com"]; !ok {
			t.Fatalf("bob missing")
		} else {
			if bob.Cookie != "cookie_b=yyy; UID=u2" {
				t.Errorf("bob cookie mismatch: %q", bob.Cookie)
			}
		}
		...
		if err != nil {
			t.Fatalf("restart ReadAccounts err: %v", err)
		}
		if len(back2) != 2 {
			t.Errorf("restart accounts len want 2, got %d", len(back2))
		}
		for _, a := range back2 {
	...
	t.Run("SettingsStore defaults + round-trip + restart", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("read default settings err: %v", err)
		}
		if len(def.StrmExtensions) == 0 {
			t.Errorf("default StrmExtensions empty")
		}
		if len(def.DownloadExtensions) == 0 {
			t.Errorf("default DownloadExtensions empty")
		}
		...
		if err != nil {
			t.Fatalf("ReadSettings back err: %v", err)
		}
	...
	t.Run("TasksStore upsert/delete/atomic restart", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("initial ReadTasks err: %v", err)
		}
		if len(list) != 0 {
			t.Errorf("initial want empty, got %d", len(list))
		}
		...
		if err := ts.UpsertTask(t1); err != nil {
		...
		if err := ts.UpsertTask(*first); err != nil {
			t.Fatalf("Upsert update err: %v", err)
		}
		updated := mustFind(t, ts, "t_001")
		if updated.TargetPath != "/mnt/nas/strm/alice_v2" {
			t.Errorf("TargetPath update failed: %q", updated.TargetPath)
		}
		...
		if err := ts.UpsertTask(t2); err != nil {
			t.Fatalf("Upsert 2nd err: %v", err)
		}
		...
		if err != nil {
			t.Fatalf("restart ReadTasks err: %v", err)
		}
		if len(list2) != 2 {
			t.Fatalf("restart tasks len want 2, got %d", len(list2))
		}

		// Delete
		ok, err := ts.DeleteTask("t_002")
		if !ok || err != nil {
			t.Errorf("DeleteTask ok=%v err=%v", ok, err)
		}
		remaining, err := ts.ReadTasks()
		if err != nil {
			t.Fatalf("ReadTasks after delete err: %v", err)
		}
		...
		if ok || err != nil {
			t.Errorf("Delete ghost ok=%v err=%v (want false/nil)", ok, err)
		}
	})
}
