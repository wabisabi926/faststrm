...
func TestPhase4_RateIntegration(t *testing.T) {
	t.Run("Limiter: 60 QPM bucket → 60 immediate Acquires, 61st times out, then refill works", func(t *testing.T) {
		...
		// 初始应有 60 令牌
		if n := lim.AvailableTokens(); n != 60 { t.Fatalf("initial tokens want 60, got %d", n) }

		// 60 次立即成功
		for i := 0; i < 60; i++ {
			if err := lim.Acquire(ctx); err != nil { t.Fatalf("Acquire[%d] err: %v", i, err) }
		}
		if n := lim.AvailableTokens(); n != 0 { t.Fatalf("after 60 Acquire, tokens want 0, got %d", n) }
		...
		if n := lim.AvailableTokens(); n < 1 { t.Errorf("after 1.1s want >=1 token, got %d", n) }
		// 再获取应成功
		if err := lim.Acquire(ctx); err != nil { t.Fatalf("after refill Acquire err: %v", err) }
	})
	...
	t.Run("Bottleneck(3): 4 goroutines → Running never exceeds 3", func(t *testing.T) {
		...
		run := func() {
			defer wg.Done()
			ctx := context.Background()
			if err := bn.Enter(ctx); err != nil { return }
			defer bn.Leave()
			cur := int32(bn.Running())
			for {
				old := atomic.LoadInt32(&maxRunning)
				if cur <= old || atomic.CompareAndSwapInt32(&maxRunning, old, cur) { break }
			}
		...
		if counter != 4 { t.Errorf("want 4 workers complete, got %d", counter) }
		...
		if !bn.TryEnter() { t.Fatalf("TryEnter should succeed after all Leave") }
		if bn.Running() != 1 { t.Errorf("Running after TryEnter want 1, got %d", bn.Running()) }
		bn.Leave()
		if bn.Running() != 0 { t.Errorf("Running after Leave want 0, got %d", bn.Running()) }
	})
	...
	t.Run("Registry: per-account-type lazy create + reuse + SetDefaults", func(t *testing.T) {
		...
		if l1 != l2 { t.Fatalf("same (u1,api115) want same Limiter") }
		l3 := r.GetLimiter("u1", TypeDownload)
		if l1 == l3 { t.Errorf("different type should NOT be same Limiter") }

		// SetDefaults → 新 account 应该拿新默认 api115QPM=30
		r.SetDefaults(30, 0, 2)
		lNew := r.GetLimiter("u_new", TypeAPI115)
		if n := lNew.AvailableTokens(); n != 30 { t.Errorf("new api115 QPM after SetDefaults want 30, got %d", n) }

		// Bottleneck：相同 (u2,proxy) → 同一个；新默认 concurrent=2
		b1 := r.GetBottleneck("u2", TypeProxy, 0)
		b2 := r.GetBottleneck("u2", TypeProxy, 0)
		if b1 != b2 { t.Fatalf("same Bottleneck expect reuse") }
		if b1.Limit() != 8 {
