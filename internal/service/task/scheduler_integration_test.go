...
func TestPhase4_SchedulerRuntimeIntegration(t *testing.T) {
	t.Run("scheduleToCron: 3 modes + invalid cases", func(t *testing.T) {
		...
		for _, c := range cases {
			t.Run(c.name, func(t *testing.T) {
				...
				if got != c.want { t.Errorf("scheduleToCron(%+v) want %q, got %q", c.sc, c.want, got) }
			})
		}
	})
	...
	t.Run("Scheduler Register/Status/Unregister + RefreshAll(TasksRW)", func(t *testing.T) {
		...
		// 注册前 Status 空
		st0 := sched.Status()
		if st0.Initialized || st0.Registered != 0 { t.Errorf("empty sched want no init / 0 jobs: %+v", st0) }
		...
		st1 := sched.Status()
		if st1.Registered != 2 { t.Fatalf("registered want 2, got %d: %+v", st1.Registered, st1) }
		ids := make(map[string]bool)
		for _, m := range st1.Tasks { ids[m["taskId"].(string)] = true }
		if !ids["t_iv"] || !ids["t_dy"] { t.Errorf("registered ids missing: %v", ids) }

		// Unregister
		sched.Unregister("t_iv")
		if sched.Status().Registered != 1 { t.Errorf("after unregister want 1") }
		...
		after := sched.Status()
		if after.Registered != 2 { t.Fatalf("after refresh want 2 (t1,t2), got %d: %+v", after.Registered, after) }
		ids2 := map[string]bool{}
		for _, m := range after.Tasks { ids2[m["taskId"].(string)] = true }
	...
	t.Run("Runtime StateManager: account mutex + disk persist", func(t *testing.T) {
		...
		if !res.Ok { t.Fatalf("first enter for alice should OK, got reason=%s", res.Reason) }
		...
		if !res3.Ok { t.Fatalf("bob enter should OK, got %+v", res3) }

		// d) IsAccountInFullScan 观测
		if !sm.IsAccountInFullScan("alice@115.com") { t.Error("alice should be in fullscan") }
		if !sm.IsAccountInFullScan("bob@115.com")   { t.Error("bob should be in fullscan") }
		...
		if !sm.IsMonitorSuspended("alice@115.com") { t.Error("alice monitor should be suspended now") }
		...
		if sm.IsMonitorSuspended("bob@115.com") { t.Error("bob monitor should NOT be suspended") }

		// f) TouchHeartbeat + Exit 释放 alice
		sm.TouchFullScanHeartbeat("alice@115.com")
		sm.ExitFullScan("alice@115.com")
		if sm.IsAccountInFullScan("alice@115.com") { t.Error("after exit, alice should NOT be in fullscan") }
		...
		if sm.IsMonitorSuspended("alice@115.com") { t.Error("after ClearMonitorSuspend should not suspended") }
		...
		if err != nil { t.Fatalf("read runtime.json err: %v", err) }
		if data == "" {
