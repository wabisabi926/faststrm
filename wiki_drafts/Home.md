...
> - 🪄 **修复 SSL 证书验证失败**：安装时可修改服务端口（默认 8090，升级不丢失）
> - 🧩 **新增三级路径解析降级机制**：仅保留 64/256 两种尺寸，删除多余副本
> - 🗑️ **API 域名轮换规避风控**：远程路径不再自动填入无效本地路径
> - 📋 **Cookie 认证流程修复**：日志功能恢复正常，未执行任务友好提示
> - 📊 **默认轮询间隔调整为 10 秒**：异常标红 + 115 API 域名/路径更新
