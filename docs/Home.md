...
- **孤儿文件扫描**：STRM 指向的 pickcode 在 115 不存在时，自动移入 7 天回收站
...
- **Dry-run 模式**：首次配置只记日志不实际删除
- STRM + 字幕 / nfo / 图片 + 空目录 + DB 记录，全部走 7 天回收站
...
| 数据质量 | **[STRM 清理对账](功能详解-STRM清理对账)** | 孤儿扫描、全量对账、监控暂停、回收站 |
