  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold">任务管理</h1>
          <p className="text-muted-foreground mt-1 text-sm">管理和监控你的下载任务</p>
        </div>
      ...
      >
        <DialogContent className="max-w-[95vw] sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {deleteDialogOpen && (
                <>
                  <span className="text-sm text-gray-500 mt-2 block">
                    任务ID: {deleteDialogOpen.slice(0, 8)}...
                  </span>
                  <span className="text-sm text-gray-500 mt-1 block">
                    目标路径: {data.find(t => t.id === deleteDialogOpen)?.targetPath}
                  </span>
                </>
              )}
            </DialogDescription>
          ...
          {deleteDialogOpen && (
            <div className="flex items-center gap-2 py-2">
              <Checkbox
                id={`clean-strm-mobile`}
                checked={deleteCleanStrm}
                onCheckedChange={(checked) => setDeleteCleanStrm(checked === true)}
              />
              <label htmlFor={`clean-strm-mobile`} className="text-sm cursor-pointer">
                同时清理 STRM 目录
              </label>
            </div>
          )}
          <DialogFooter className="gap-2">
            ...
            <Button
              ...
              onClick={() => {
                if (deleteDialogOpen) {
                  deleteTask(deleteDialogOpen, deleteCleanStrm);
                  setDeleteDialogOpen(null);
                  setDeleteCleanStrm(false);
                }
              }}
      ...
      {/* Clear directory dialog */}
      <Dialog
        open={!!clearDialogOpen}
        onOpenChange={(open) => setClearDialogOpen(open ? clearDialogOpen : null)}
      >
        <DialogContent className="max-w-[95vw] sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>确认清空目录</DialogTitle>
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setClearDialogOpen(null)}>
              取消
            </Button>
            <Button
              variant="destructive"
              onClick={() => {
                if (clearDialogOpen) {
                  clearDirectory(clearDialogOpen);
                  setClearDialogOpen(null);
                }
              }}
            >
